...
class GPT2Attention(nn.Module):
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            query = self.q_attn(hidden_states, self.q_attn)
...
class GPT2FlashAttention2(GPT2Attention):
    ...
    def forward(
        ...
        attn_output = self.c_proj(attn_weights_reshaped,task_types=task_types)
        attn_output = self.resid_dropout(attn_output)
...
class GPT2MLP(nn.Module):
    ...
    def forward(self, hidden_states: Optional[Tuple[torch.FloatTensor]], task_types= None) -> torch.FloatTensor:
        hidden_states = self.c_fc(hidden_states)
        hidden_states = self.act(hidden_states)
        hidden_states = self.c_proj(hidden_states, task_types=task_types)
...
class GPT2Block(nn.Module):
    ...
    def forward(
        ...
        feed_forward_hidden_states = self.mlp(hidden_states, task_types=task_types)
        # residual connection
